...
import { Typography, TextField , Button,Checkbox, makeStyles } from "@material-ui/core";
...
const AddTransactions = () => {
	...
	return (
		<form>
			<Typography className={classes.heading} variant="h5">Add a transaction</Typography>
			...
			<TextField className={classes.inputs} fullWidth onChange={(e) => setTitle(e.target.value)} value={title} label="Enter a discription" />
			<TextField className={classes.inputs} fullWidth onChange={(e) => setPrice(e.target.value)} value={price} label="Enter a amount" />
			<Button 
			fullWidth 
			variant="contained" 
			color="secondary"
